...
export async function startDiscordBot({ token, publicUrl, io }) {
  ...
  async function applySkinovaIdentity(ready) {
    try {
      ready.user.setPresence({
        activities: [{ name: 'Skinova · /skinova', type: ActivityType.Playing }],
        status: 'online',
      });
    } catch {}
    for (const guild of ready.guilds.cache.values()) {
      try {
        ...
        if (me.permissions.has(PermissionFlagsBits.ChangeNickname) && me.nickname !== 'Skinova') {
          await me.setNickname('Skinova', 'Identité du bot Skinova');
        }
      } catch (error) {
        console.warn(`[Discord] Impossible de renommer le bot sur ${guild.name}: ${error?.message || error}`);
      }
    }
  }
  ...
  client.once(Events.ClientReady, async (ready) => {
    ...
    await applySkinovaIdentity(ready);
    console.log('[Discord] Installateur prêt : /setup-skinova');
  });

  client.on(Events.GuildCreate, async (guild) => {
    try {
      const me = guild.members.me || await guild.members.fetchMe();
      if (me.permissions.has(PermissionFlagsBits.ChangeNickname)) await me.setNickname('Skinova');
    } catch {}
  });
